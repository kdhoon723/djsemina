<template>
  <div class="timeline-card">
    <!-- 요약 + 범례 (스크롤 영역 밖, 고정) -->
    <div class="tl-head">
      <span class="tl-summary"><span class="dot"></span>{{ summaryText }}</span>
      <div class="tl-legend">
...
// "지금" 커서가 트랙 왼쪽 끝(방이름 칸 바로 오른쪽)에 오도록 가로 스크롤
function scrollToNow() {
  const sc = scrollEl.value;
  if (!sc || nowPct.value === null) return;
  const now = sc.querySelector(".tl-now");
  const room = sc.querySelector(".tl-room");
  if (!now) return;
  const scLeft = sc.getBoundingClientRect().left;
  const nowLeft = now.getBoundingClientRect().left;
  const roomW = room ? room.getBoundingClientRect().width : 0;
  sc.scrollLeft += nowLeft - (scLeft + roomW);
}
onMounted(() => nextTick(() => requestAnimationFrame(scrollToNow)));
watch(() => [props.rooms, props.date], () => nextTick(() => requestAnimationFrame(scrollToNow)));
...
.tl-inner { min-width: 100%; }

.tl-axis-row { display: grid; grid-template-columns: var(--rooml) 1fr; margin-bottom: 6px; }
.tl-room-sp { position: sticky; left: 0; z-index: 6; background: var(--card-bg); }
.tl-axis-track { position: relative; height: 16px; }
.tl-tick { position: absolute; transform: translateX(-50%); font-size: 0.68rem; color: var(--text-light); font-weight: 600; }
.tl-now-tick { position: absolute; transform: translateX(-50%); font-size: 0.62rem; font-weight: 700; color: #fff; background: #ef4444; padding: 1px 5px; border-radius: 999px; white-space: nowrap; top: -1px; z-index: 4; }

.tl-rows { display: flex; flex-direction: column; }
.tl-row { display: grid; grid-template-columns: var(--rooml) 1fr; align-items: center; padding: 5px 0; }
.tl-row:not(:last-child) { border-bottom: 1px solid #f1f5f9; }
.tl-room { position: sticky; left: 0; z-index: 5; background: var(--card-bg); display: flex; flex-direction: column; line-height: 1.2; padding-right: 8px; min-width: 0; border-right: 1px solid #eef2f7; }
...
/* 모바일: 30분 단위가 읽히도록 확대(가로 스크롤). 데스크톱은 전체 한 화면. */
@media (max-width: 600px) {
  .timeline-card { --rooml: 60px; }
  .tl-inner { min-width: 1160px; }
  .tl-room-name { font-size: 0.78rem; }
  .tl-room-type { font-size: 0.6rem; }
  .tl-tick { font-size: 0.62rem; }
